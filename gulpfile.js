...
function watchFiles() {
  ...
  gulp.watch(
    ...
    gulp.series(scripts, jsVersions, css, cssVersions)
  );
  gulp.watch(
  ...
  gulp
    .watch("src/images/**/*.{png,jpg}", gulp.series(tinyPngImages))
  ...
  gulp
    .watch("src/favicon/*.{png,jpg}", tinyPngImagesFavicon)
    .on("unlink", function (filepath) {
...
gulp.task(
  ...
  gulp.series(
    gulp.parallel(
      gulp.series(css, cssVersions, cssPrint),
      gulp.series(scripts, jsVersions),
      gulp.series(twigTemplates, htmlBeauty, indexInject)
    ),
